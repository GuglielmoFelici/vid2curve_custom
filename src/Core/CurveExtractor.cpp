...
void CurveExtractor::RunPcaGPU(int trunc_r, double r) {
    ...
    for (int x = 0; x < height_; x++) {
        for (int y = 0; y < width_; y++) {
            ...
            Eigen::Matrix2d M;
            M << out_data_pool[idx * 4], out_data_pool[idx * 4 + 1], out_data_pool[idx * 4 + 2], out_data_pool[idx * 4 +
                                                                                                               3];
            double l0 = 0, l1 = 0;
            Eigen::Vector2d v0, v1;
            CalcEigens(M, l0, l1, v0, v1);
    ...
    if (use_average_smoothing) {
        ...
        for (const auto &path : paths_) {
            ...
            for (int i = 1; i + 1 < path.size(); i++) {
                new_points[path[i]] =
                        0.25 * points_[path[i - 1]] + 0.5 * points_[path[i]] + 0.25 * points_[path[i + 1]];
            }
        }
        points_ = new_points;
        for (int i = 0; i < n_points_; i++) {
    ...
    if (std::abs(polluted_radius) > 1e-3) {
        ...
        for (const auto &pt : polluted_positions) {
            ...
            for (int i = 0; i < n_slices; i++) {
                double new_r =
                        i + 1 == n_slices ? origin_r : (std::rand() / double(RAND_MAX) * 0.7 + 0.3) * polluted_radius;
                double L = ang_step * i;
