...
void Model::FeedExtractor(CurveExtractor *extractor) {
    ...
    for (auto initial_flow :
            std::vector<Eigen::Vector2d>{Eigen::Vector2d::Zero(),
                                         new_view->view_center_ - views_.back()->view_center_}) {
...
void Model::OutputProcedureMesh() {
    ...
    for (const auto &active_view : views_) {
        ...
        while (view_pool_iter->get() != active_view) {
            ...
            current_view->
                    Track(pose_candidates, points_, points_history_, tangs_, tang_scores_, curve_network_,
                          spanning_tree_, octree_);
...
bool Model::IsCameraMotionSimilar(Model *ano_model) {
    ...
    for (int i = 0; i + 1 < views_.size(); i++) {
        ...
        Eigen::Vector3d ano_prev_at_next =
                ano_model->views_[i + 1]->camera_.World2Cam(
                        -ano_model->views_[i]->R_.inverse() * ano_model->views_[i]->T_);
    ...
    if (use_uniform_resample) {
        ...
        for (const auto &path : paths) {
            ...
            for (int i = 0; i + 1 < path.size(); i++) {
                ...
                for (const auto &view : views_) {
                    Eigen::Vector2d bias =
                            view->camera_.World2Image(points_[path[i]]) -
                            view->camera_.World2Image(points_[path[i + 1]]);
                    current_sampling_ratio = std::max(current_sampling_ratio,
                                                      std::max(std::abs(bias(0)), std::abs(bias(1))));
                }
    ...
    if (use_lsqr) {
            ...
            if (smooth_method_ == "TANGENT") {
                ...
                for (int i = 1; i + 1 < indexes.size(); i++) {
                    double ratio = std::abs(
                            (points_[indexes[i]] - points_[indexes[i - 1]]).dot(
                                    points_[indexes[i + 1]] - points_[indexes[i - 1]]) /
                            (points_[indexes[i]] - points_[indexes[i + 1]]).dot(
                                    points_[indexes[i + 1]] - points_[indexes[i - 1]]));
        ...
        for (int i = 1; i + 1 < indexes.size(); i++) {
            ...
            for (int t = 0; t < 3; t++) {
                ...
                ceres::CostFunction *cost_function =
                        SmoothnessError::Create(hope_dist_weight_ * hope_dist_ / len_mean,
                                                hope_dist_weight_ * hope_dist_ / len_mean);
        ...
        for (int idx : indexes) {
            ...
            for (const auto &pr : valid_views) {
                ...
                View *view = pr.second;
                current_error +=
                        weight * (view->matching_pixels_[idx] - view->camera_.World2Image(points_[idx])).norm();
            }
        ...
        for (int i = 1; i + 1 < curve->points_p_.size(); i++) {
            ...
            for (int t = 0; t < 3; t++) {
                idx++;
                ceres::CostFunction *cost_function =
                        SmoothnessError::Create(hope_dist_weight_ * hope_dist_ / len_a,
                                                hope_dist_weight_ * hope_dist_ / len_b);
                problem.AddResidualBlock(cost_function,
                                         nullptr,
...
void Model::Update3DRadius(double extend_2d_radius) {
    int view_idx = 0;
    int n_views = views_.size();
    auto tree = std::make_unique<SpanningTree>(points_, octree_, hope_dist_ * topology_searching_radius_, hope_dist_,
                                               4);
#pragma omp parallel for private(view_idx) default(none) shared(n_views, points_, tangs_, tang_scores_, hope_dist_, views_, tree)
    for (view_idx = 0; view_idx < n_views; view_idx++) {
        const auto &view = views_[view_idx];
...
void Model::RadiusSmoothing(Graph *graph) {
    ...
    for (const auto &path : paths) {
        for (int i = 0; i < path.size(); i++) {
            int u = path[i];
            near_junction_ratio[u] = std::min(near_junction_ratio[u],
                                              double(std::min(i, int(path.size()) - 1 - i)) / path.size());
        }
    }
...
#pragma omp parallel for private(view_idx) default(none) shared(views_)
    ...
    while (views_.size() > ::kMaxKeyFrameN) {
        ...
        if (remove_by_time_scale) {
            ...
            for (int view_i = 1;
                 view_i + kSlidingWindowN < views_.size(); view_i++) { // Don't remove the first one and the last one.
                ...
                for (int view_j = 1; view_j + kSlidingWindowN < views_.size(); view_j++) {
                    ...
                    for (int u : sampled_u) {
                        if (views_[view_i]->GetMatchingRadius(u) < 0.0 || views_[view_j]->GetMatchingRadius(u) < 0.0) {
                            if (views_[view_i]->GetMatchingRadius(u) > 0.0 &&
                                views_[view_j]->GetMatchingRadius(u) < 0.0) {
                                valid_base += 1.0;
                                similarity += -1.0;
                            }
                            continue;
                        }
                        Eigen::Vector3d ray_i = (views_[view_i]->R_.inverse() *
                                                 views_[view_i]->GetMatching(u).first).normalized();
                        Eigen::Vector3d ray_j = (views_[view_j]->R_.inverse() *
                                                 views_[view_j]->GetMatching(u).first).normalized();
                        valid_base += 1.0;
