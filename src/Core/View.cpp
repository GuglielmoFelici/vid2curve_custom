    ...
    for (const auto &pose_candidate : pose_candidates) {
        ...
        for (int try_t = 0; try_t < kRandomTryingTime && how_good_tracking_is <= 0; try_t++) {
            ...
            do {
                ...
                how_good_tracking_is = this->TrackSingleStep(points, current_tracking_indexes,
                                                             current_matching_indexes);
...
int View::TrackSingleStep(const std::vector<Eigen::Vector3d> &points,
    ...
    for (int i = 0; i < tracking_indexes.size(); i++) {
        double dis =
                (camera_.World2Image(points[tracking_indexes[i]]) -
                 extractor_->points_[current_matching_indexes[i]]).norm();
        if (dis > 3.0) {
            n_bad_points++;
        }
        if (dis < 1.1) {
...
void View::CacheMatchings(const std::vector<Eigen::Vector3d> &world_points,
    ...
    // DP.
    {
        ...
        DFS = [&DFS, &for_dp, &neighbors, &f, &points, &states, this]
                (int u, int i, int n, int state, double dis, Eigen::Vector2d current_bias,
                 const Eigen::Vector2d &hope_bias) -> void {
            if (i == n) {
            ...
            for (int idx = 0; idx < neighbors[v].size(); idx++) {
                ...
                DFS(u, i + 1, n, state * K + idx, dis + f[v][idx], current_bias - weight * points[candidate.first],
                    hope_bias);
            }
        };
    ...
    if (show_debug_info) {
        ...
        for (int u = 0; u < n_world_points; u++) {
            ...
            if (cnt++ % 10 == 0) {
                cv::line(img, cv::Point(x_0 * 2.0, y_0 * 2.0), cv::Point(x_1 * 2.0, y_1 * 2.0), cv::Scalar(200, 0, 200),
                         1.5);
            }
        }
...
void View::GetMissingWorldRays(const std::vector<Eigen::Vector3d> &points,
    ...
    for (int idx = 0; idx < n_points_; idx++) {
        ...
        if (!found) {
            Eigen::Vector3d ray((point(1) - 0.5 * width) / focal_length_, (point(0) - 0.5 * height) / focal_length_,
                                1.0);
            ray /= ray.norm();
