...
void Initializer::GetInitialModelData(std::vector<std::unique_ptr<ModelData>> *model_states) {
            ...
            for (int z = 0; z <= 0; z++) { // ?? -guglielmo
                ...
                for (const auto &matching : matchings) {
                    ceres::CostFunction *cost_function =
                            ProjectionError::Create(matching, (matching.q.score_ - 1.0) * (-2.0), focal_length_, width_,
                                                    height_);
                ...
                double final_single_depth_error_weight =
                        single_depth_error_weight_ / kInitialDepth * focal_length_ * matchings.size() /
                        extractor_0->n_points_;
                for (int i = 0; i < extractor_0->n_points_; i++) {
                    ceres::CostFunction *cost_function = SingleDepthError::Create(kInitialDepth,
                                                                                  final_single_depth_error_weight);
                ...
                for (int i = 0; i < extractor_0->n_points_; i++) {
                    new_model_state->points.emplace_back(
                            Utils::ImageCoordToCamera(extractor_0->candi_points_[i].o_, height_, width_,
                                                      focal_length_) * depths[i]);
                }
            }
        }
    }
    ...
    if (average_trans <
        0.03) { // Dovrebbe essere il treshold di cui si parla a pag 0:6, quindi potrebbe essere la distanza della camera nei due frame considerati
        model_states->clear();
    }
