...
void Graph::GetLinkedPaths(const std::vector<Eigen::Vector3d> &points,
    ...
    for (int path_idx = 0; path_idx < original_paths.size(); path_idx++) {
        ...
        outs[path.back()].emplace_back(path_idx, (points[path[path.size() - step - 1]] -
                                                  points[path[path.size() - header_idx - 1]]).normalized());
    }
    ...
    for (int u = 0; u < n_points_; u++) {
        ...
        for (const auto &pr : edges_[u]) {
            ...
            while (Degree(v) == 2) {
                ...
                CHECK(new_v != v) << v << " " << new_path.back() << " " << edges_[v][0].first << " "
                                  << edges_[v][1].first;
