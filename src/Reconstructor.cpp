    ...
    if (reconstruction_method_ == "POSE_CANDIDATES") {
        ...
        for (int i = 1; i < 10 && i < n_views_; i++) {
            ...
            auto pose_initializer =
                    std::make_unique<Initializer>(std::vector<CurveExtractor *>{
                                                          extractors_[0].get(),
                                                          extractors_[i].get()},
                                                  focal_length_,
                                                  (double) width_,
        ...
        for (int frame_id = initial_frame_id + 1; frame_id <= initial_frame_id + kMaxTryFrameN; frame_id++) {
            ...
            if (frame_id > initial_frame_id + kMinTryFrameN &&
                new_models.begin()->second->IsCameraMovementSufficient()) {
...
void Reconstructor::PressureTest(Model *final_model) {
    ...
    for (int left_idx = 0; left_idx < original_n_views; left_idx += 10) {
        ...
        auto left_extractor = std::make_unique<CurveExtractor>((StreamerBase *) left_single_image_streamer.get(),
                                                               &ptree_);
        ...
        for (auto track_type : std::vector<TrackType>{TrackType::MATCHING, TrackType::NAIVE}) {
            ...
            for (int right_idx = left_idx + 1; right_idx < left_idx + 500;) {
                ...
                if (step < 1) {
                    LOG(INFO)
                            << (track_type == TrackType::NAIVE ? "NAIVE" : (track_type == TrackType::MATCHING ? "MATCH"
                                                                                                              : "MATOF"))
            ...
            LOG(INFO) << "average: "
                      << (track_type == TrackType::NAIVE ? "NAIVE" : (track_type == TrackType::MATCHING ? "MATCH"
                                                                                                        : "MATOF"))
                      << " " << std::accumulate(tracking_steps[u].begin(), tracking_steps[u].end(), 0.0) /
                                tracking_steps[u].size();
        }
    }
